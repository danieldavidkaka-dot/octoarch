Entendido, Arquitecto. Mantenemos el v5.0 como la versión oficial en el documento. He integrado los nuevos protocolos de seguridad y límites de iteración directamente en tu estructura original sin alterar la numeración.

Aquí tienes el Contexto Global definitivo con la "Jaula de Titanio" y el comando octo sw incorporados:
...
Motor Cognitivo Unificado (El CEO y El Obrero): Google Gemini API (gemini-3.0-flash / Pro) para orquestación empática de alta velocidad, razonamiento complejo y forjado de código sin latencia de cambio de contexto.
...
Seguridad (RBAC, Secretos y Path Jailing): * Control de Acceso Basado en Roles estricto (Roles bloqueados como CFO, CHAT no pueden usar terminal ni forjar código).
...
Validación de Rutas: El sistema de archivos aplica un escudo contra Path Traversal. Está terminantemente prohibido acceder a rutas fuera del workspace definido.

4. Flujos Principales de Negocio
4.1. Auto-Programación (Sistema Swarm y Bounded Autonomy)
Cuando OctoArch necesita una herramienta técnica nueva, el Orquestador invoca al Obrero. El Obrero escribe el código usando una plantilla estricta (con formato de etiquetas XML) y lo guarda en src/temp_tools/.

Control de Iteraciones: El Obrero opera con un límite de 3 intentos de compilación por defecto para asegurar eficiencia.

Comando octo sw: Solo si el usuario autoriza explícitamente mediante el prefijo octo sw, el Obrero tiene permitido extender su autonomía hasta 5 intentos para resolver problemas de lógica compleja o errores de compilación persistentes.

Una vez superadas las pruebas de sintaxis, se mueve a src/dynamic_tools/. El DynamicRegistry detecta el cambio e inyecta la nueva herramienta al Cerebro en tiempo real (Hot-Swap).

4.2. Flujo Cognitivo Centralizado
OctoArch actúa como Director General utilizando un único núcleo (Gemini). Resuelve las consultas generales con extrema velocidad y empatía. Cuando se enfrenta a problemas de lógica pura o código complejo, delega la tarea a su "Sub-Mente" (el Sistema Swarm), garantizando la máxima precisión sin alucinaciones.
